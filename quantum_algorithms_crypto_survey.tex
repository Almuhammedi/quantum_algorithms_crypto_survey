...
\textbf{Keywords—} Post-Quantum Cryptography (PQC), Cryptanalytically Relevant Quantum Computer (CRQC), Quantum Algorithms, Shor’s Algorithm, Grover’s Algorithm, ML-KEM, ML-DSA, SLH-DSA, NIST FIPS 203–205, Lattice-Based Cryptography, Crypto-Agility, Harvest-Now-Decrypt-Later (HNDL), Lightweight Cryptography, Foundational Mathematical Risks.
...
Shor's algorithm provides a polynomial-time quantum method for factoring and discrete logarithms. Contemporary resource estimates incorporating error correction indicate rapid progress: Gidney estimates sub-million-physical-qubit factoring of RSA-2048 in under a week (under specific hardware assumptions), while earlier baselines required tens of millions of qubits; QLDPC-based proposals (\emph{Pinnacle}) claim further reductions to about $\sim 10^5$ physical qubits at the cost of tougher architectural demands and longer runtimes.\cite{shor1997,gidney2025,webster2026}
...
NIST key‑management recommendations and cost analyses provide baseline
guidance for selecting symmetric-key parameters suitable for long‑lived
data in the presence of quantum adversaries, including the use of AES‑256
and SHA‑384/512 to retain adequate post‑quantum security margins.\cite{sp80057}
...
\begin{table}[h]
\centering
\begin{tabular}{@{}p{2.5cm}p{3.5cm}p{7.3cm}@{}}
\toprule
...
FIPS 204 \\(ML-DSA) & Signature (Module-SIS/MLWE) & Dilithium; general-purpose PQ signatures with strong implementation guidance.\\
FIPS 205 \\(SLH-DSA) & Signature (Hash-based) & SPHINCS+; large signatures but structurally conservative fallback.\\
FIPS 206 \\(FN-DSA) & Signature (NTRU-lattice) & Falcon-like; draft in progress with floating-point/validation caveats.\\
...
Recent reductions in qubit estimates for RSA-2048 factoring (from \(\sim\!20\) million to below one million, and potentially to \(\sim\!10^5\) under QLDPC assumptions) compress perceived timelines for CRQC feasibility.\cite{gidney2025,webster2026} Given HNDL, long-lived data encrypted today with vulnerable public-key schemes may be at risk if not rekeyed to PQC.\cite{hndl-fed}
...
Given Grover’s quadratic speedup for key search, the effective post-quantum
security of a $k$-bit symmetric key is reduced to roughly $k/2$ bits. This
motivates the use of AES-256 for long-lived confidentiality, as it maintains
approximately 128 bits of post-quantum strength. 

For hash-based constructions, it is important to distinguish preimage and
collision security. Grover’s algorithm reduces preimage resistance from $n$
bits to $2^{n/2}$ operations, but collision resistance is governed not by
Grover but by the Brassard–Høyer–Tapp (BHT) quantum collision algorithm,
which reduces the cost of collision search to approximately $2^{n/3}$.
Consequently, shorter digests such as SHA-256 provide limited long-term
collision security in the quantum setting, whereas SHA-384/512 retain
adequate margins. Consistent with NIST’s key-management recommendations,
AES-256 and SHA-384/512 are preferred for long-lived post-quantum protection.\cite{bht1997}

\section{Conclusions}
Quantum cryptanalysis places many classical public-key algorithms on a deprecation path. Rapid progress in resource estimates and the emergence of standardized PQC building blocks underscore the urgency of migration.\cite{gidney2025,webster2026} The NIST PQC standards (FIPS~203--205) provide stable primitives; FIPS~206 and HQC will further improve algorithmic diversity as deployments mature.\cite{nist203,nist204,nist205,fips206status,nistpqc}
...
Because two of the three finalized NIST PQC standards are lattice-based (ML-KEM and ML-DSA), early deployments may concentrate risk in a single mathematical family; maintaining algorithmic diversity (e.g., via SLH-DSA and forthcoming HQC/FN-DSA) mitigates correlated failure modes.\cite{nist203,nist204,nist205,fips206status,nistpqc}
