...
\begin{abstract}
The rapid advancement of quantum computing poses a fundamental threat to classical cryptographic systems based on integer factorization and discrete logarithms. This paper presents a comprehensive survey of quantum cryptanalytic algorithms and their implications for modern security infrastructures. The paper reviews state-of-the-art resource estimates for quantum factoring, including recent approaches that reduce physical qubit requirements under optimistic assumptions, as well as alternative frameworks such as Regev’s algorithm. The impact of quantum search algorithms on symmetric primitives is also analysed, highlighting practical reductions in effective security levels.

In parallel, the paper examines the status of post-quantum cryptographic standardisation, with emphasis on NIST FIPS 203–205 and ongoing developments. Implementation-level vulnerabilities, including recent timing attacks on lattice-based schemes, are discussed to underscore deployment challenges.

Finally, a risk assessment framework based on Mosca’s inequality is applied to evaluate migration timelines under the “harvest now, decrypt later” threat model. The analysis highlights that risk is driven not only by uncertainty in quantum capabilities but also by the accumulation of vulnerable encrypted data. Based on these findings, practical recommendations are provided to support cryptographic agility and secure transition to post-quantum standards.
\end{abstract}

\textbf{Keywords—} Cryptanalytically Relevant Quantum Computer, Post-Quantum Cryptography, Shor’s Algorithm, Grover’s Algorithm, NIST FIPS 203–205.
...
\begin{table}[h]
\centering
\caption{Comparison of major quantum computing hardware platforms (as of 2026)}
...
Gate Speed & $\sim$10--100 ns & ns--$\mu$s & TBD \\
...
Despite these advances, the precise realization date of a CRQC remains uncertain. Public roadmaps from major hardware developers suggest progress toward early fault-tolerant systems within the next decade, but translating these into large-scale, application-capable machines introduces significant additional engineering challenges. As such, estimates of $z$ on the order of 10–20 years should be interpreted as optimistic and highly assumption-dependent.

The risk captured by Mosca’s Inequality is further amplified by the HNDL threat model, wherein adversaries systematically collect encrypted data today ($x$) with the expectation of future decryption. Within this framework, the migration timeline ($y$) assumes critical importance: if $x + y \geq z$, even a delayed quantum breakthrough can result in retrospective compromise of sensitive information. Consequently, the migration window should not be treated as a future contingency, but rather as a present operational constraint for data with long-term confidentiality requirements.

More fundamentally, Mosca’s formulation captures a threshold condition but does not fully account for the continuous accumulation of exposed ciphertext during the migration period. In practice, post-quantum risk is driven not solely by uncertainty in CRQC timelines (z), but by the deterministic growth of vulnerable data as migration is deferred. This accumulation effect systematically increases the potential impact of HNDL adversaries, irrespective of the precise timing of large-scale quantum capabilities.
...
Finally, recent FIPS publications related to post-quantum cryptography (FIPS~203--205) focus exclusively on asymmetric primitives, while the forthcoming FIPS~206 addresses digital signatures. None of these introduce new lightweight or stream cipher standards, reinforcing the observation that symmetric cryptography within FIPS remains centered on AES rather than newer lightweight or software-optimized designs.
...
\textit{Applied Physics Reviews}, 2019.
